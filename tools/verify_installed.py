...
#: The plugin this repository *is*, read off the manifest rather than typed.
SOURCE_MANIFEST = Path(__file__).resolve().parents[1] / "hermes-kame-api-rotation" / "plugin.yaml"


def _plugin_version() -> str:
    """The version in the source manifest, or ``""`` if it cannot be read."""
    try:
...
def main() -> int:
    ...
    if module_name:
        ...
        # 1.7.0.2: this read `"1.6.0" in report` and had been failing silently
        # since 1.7.0.0 shipped — a tripwire pinned to a literal version fails
        # on every release *after* the one that wrote it, which is the opposite
        # of what a tripwire is for. What it is actually checking is that the
        # report names *a* build, so it now asks the source what that is.
        installed = _plugin_version()
        check(
            f"it names the running build ({installed})",
            bool(installed) and installed in report,
            True,
        )
