        ...
        while True:
            ...
            try:
                ...
                if verdict == "raise":
                    ...
                    # --- ABSOLUTE SHIELD (1.3.0) ---
                    # Intercept non-rotational errors so Hermes doesn't do its 3 retries
                    # and abort instantly, showing a clean message in the chat.
                    from types import SimpleNamespace
                    msg = f"KAME [Absolute Shield]: The request was refused by the provider (HTTP {status or 'Error'}). This is an unrecoverable structural error (e.g., context exceeded, model offline, or invalid parameters) and cannot be fixed by rotating keys.\n\nPlease clear the chat or switch to a different model."
                    if hasattr(last_error, "message") and last_error.message:
                        msg += f"\n\nDetails: {last_error.message}"
                    elif str(last_error):
                        msg += f"\n\nDetails: {str(last_error)}"
                        
                    logger.error(f"kame: {identity} {type(last_error).__name__} intercepted by Absolute Shield - returning synthetic response.")
                    _publish(self, None)
