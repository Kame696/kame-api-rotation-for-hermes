...
class TestTheCatalogue:
    ...
    def test_a_wastebasket_never_beats_a_fact(self):
        ...
        # RED_TEAM.md F1 (2026-09-19): 1.7.0.7 briefly moved this to
        # AUTH_REFRESH ("an opaque authentication label does not establish
        # permanent credential death"), applied through the generic catalog
        # table to every surface. Measured cost: a real Gemini 401 stopped
        # rotating and ended the turn instead. Restored to AUTH_DEAD — the
        # narrower, surface-scoped exception now lives only in
        # provider_rules.py's google_vertex rule, unreachable from look_up().
