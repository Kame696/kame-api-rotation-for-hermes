...
def _resting_pool(binding, *, seconds: float):
    ...
    1.7.0.0: the kind is ``rate_limit`` rather than ``daily``. These tests are
    about the wait notice, and they need a rest of *exactly* ``seconds`` to be
    about it — ``daily`` now means "the day is spent" and rests for the daily
    cooldown whatever number came with it, which is the point of that release
    but has nothing to do with what is being tested here.
    """
    agent = Agent()
    identity = carousel.Carousel.identity(agent.provider, agent.model)
    for key in KEYS:
        binding.engine.mark(identity, key, False, seconds, "rate_limit")
...
class TestWaitingForAKeyToComeBack:
    ...
    def test_a_long_wait_does_not_narrate_every_minute(self, monkeypatch):
        ...
        #
        # 1.7.0.0: this used to assert 12, and it passed for a reason that was
        # not the one written down. The helper benched the keys as ``daily``,
        # and the old daily branch clamped every rest to ``daily_cooldown_s``
        # — so "six hours" was silently one hour and twelve notices covered
        # it. The wait is now the six hours the test asks for, and the rule it
        # means to state is the one in its own name: far fewer notices than
        # passes. One line every ten minutes, not one a minute.
        passes = 6 * 60
        assert len(said) <= passes // 8, f"too chatty: {len(said)} notices"
