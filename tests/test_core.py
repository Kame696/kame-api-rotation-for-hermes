...
class TestTheSentenceGoogleSendsOnEveryThrottle:
    ...
    * ``account`` is in ``escalate.NEVER_STRETCH_WINDOWS`` and the reason
    ...
    def test_the_bare_sentence_with_no_wait_and_no_window_is_still_billing(self):
        """Nothing to wait for and no counter named — and this was re-argued.

        It was removed once. Google's free-tier 429 says OpenAI's
        out-of-credits words for a twenty-one second throttle, 1,876 recorded
        refusals took this branch, and the fix looked like deleting it.

        Deleting it made the plugin answer ``rate_limit`` for every shape that
        wears those words — a plain 402, an aggregator's "insufficient
        credits", a free-tier model block — and because a plugin hook answers
        before the host's classifier, answering is overriding. Four of the
        host's own tests failed, all of them cases the host had right.

        So the branch stands, with the guard it always had: only when nothing
        named a wait and the catalogue did not call it a throttle. What changed
        is the price of being wrong. This verdict used to end the turn; anything
        the pool can rotate past is now retryable, so the same misreading costs
        one rotation instead of the answer.
        """
        verdict = classify(
            provider="some-gateway", status_code=429,
            error_message=(
    ...
    def test_and_being_wrong_about_it_no_longer_ends_the_turn(self):
        # The half that actually hurt. A pool of seventeen separate accounts
        # does not stop because one of them looks spent.
        import importlib

        plugin = importlib.import_module("hermes-kame-api-rotation")
        result = plugin._on_api_error_classification(
            provider="some-gateway", status_code=429,
            error_message=(
                "You exceeded your current quota, please check your plan "
                "and billing details."
