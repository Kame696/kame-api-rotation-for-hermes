...
REQUIRED_MODULES: Tuple[str, ...] = (
    ...
    "core/stitch.py",
    "core/multikey.py",
    # RED_TEAM.md F10 (2026-09-19): core/carousel.py imports this
    # unconditionally at module level (see that file's own import list), so
    # an install missing it does not degrade — it raises ImportError the
    # moment the carousel loads, which is exactly what this list's own
    # docstring says it exists to catch before it surfaces as a traceback
    # nobody saw. Left out of the original 1.8.0.0 addition alongside
    # core/provider_rules.py above.
    "core/shared_health.py",
)
