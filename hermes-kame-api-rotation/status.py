...
class QuotaCommand:
    ...
    def _carousel(self) -> List[str]:
        ...
        # 1.8.1.0. One line, either way, so whether the doubling ladder is
        # running is something a person reads instead of infers. It lives
        # here — plain chat text from a slash command — and deliberately not
        # on the spinner: ``dispatch_binding.status_line`` is parsed by the
        # Desktop's status-row filter, and any text outside the shape it
        # expects erases the host's own line rather than being ignored.
        engine = getattr(binding, "engine", ENGINE)
        if getattr(engine, "unsized_throttle_backoff", False):
            lines.append(
                "  bare RESOURCE_EXHAUSTED backoff: ON — rests 1s, 2s, 4s… "
                f"up to {format_duration(engine.unsized_backoff_max_s)}; KAME_UNSIZED_BACKOFF=0 = flat"
            )
        else:
            lines.append(
                "  bare RESOURCE_EXHAUSTED backoff: off — flat "
                f"{format_duration(engine.unsized_throttle_rest_s)} rest"
            )
        lines.append(
