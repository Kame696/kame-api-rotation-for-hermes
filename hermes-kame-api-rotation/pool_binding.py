...
class PoolBinding:
    ...
    def _build_current(self, original: Callable) -> Callable:
        ...
        def _kame_current(pool, *args, **kwargs):
            # Forward whatever the host passes. Hermes 0.21.3 added ``model=``
            # to half of this class; a replacement that names only the
            # arguments of the version it was written against turns every
            # call from a newer host into a TypeError.
            entry = original(pool, *args, **kwargs)
            try:
    ...
    def _guard_persist(self, pool_class: Any) -> None:
        ...
        def _kame_persist(pool, *, removed_ids=None, **kwargs):
            # Hermes added status_cleared_ids for deliberate resets and OAuth
            # refresh. Forward host-owned options unchanged; the guard only
            # filters derived rows, never alters persistence intent.
            with pool._lock:
                try:
                    ...
                    return original(pool, removed_ids=removed_ids, **kwargs)
    ...
    def _carry_deadline(self, pool: Any, error_context: Any) -> Any:
        ...
        * **Only for this call.** ``peek_judgement`` matches on provider and
        ...
        # below, in the host's own pool — the real, enforced deadline, not a
        # number this plugin merely reasons about afterwards. KAME's own
        # ``judgement.reset_at`` can already be a multi-hour stated deadline
    ...
    def _remember(self, pool: Any, updated: Any, *, status_code: Any = None) -> None:
        ...
        if reset_at is not None:
            ...
            # G8: bound whatever this function is about to hold the key to,
            # including the widening just above — ``escalate.MAX_HOLD_SECONDS``
            # alone allows 24h. This cannot shorten ``reset_at`` itself below
            # what the host is already holding: ``ledger.record`` writes
    ...
    def _short_streak(self, *, credential_id: str, model: str, window: str, now: float,
                      source: Optional[str] = None, reason: Optional[str] = None) -> int:
    ...
    def _build_select_unlocked(self, original: Callable) -> Callable:
        ...
        def _kame_select_unlocked(pool, *, refresh: bool = True, **host_options: Any):
            # ``count`` (0.21.1+) and ``model`` (0.21.3+) belong to the host and
            # are forwarded untouched: ``try_refresh_matching`` passes
            # ``count=False`` and ``_select_under_lock`` passes ``model=``, and a
            # wrapper that refuses either raises on the path that hands out
            # every credential.
            binding._selecting.active = True
            try:
                result = original(pool, refresh=refresh, **host_options)
            finally:
                binding._selecting.active = False
            if host_options.get("count", True) is False:
                # A look, not a hand-out - the host's own ``count=False`` says
                # so. Charging it as load would steer the next real selection
                # away from a key nobody used.
                return result
            try:
    ...
    def _build_available_entries(self, original: Callable) -> Callable:
        ...
        def _kame_available_entries(
            pool, *, clear_expired: bool = False, refresh: bool = False, **host_options: Any
        ):
...
def _ceiling_s() -> float:
    """The owner's ceiling on any hold this module applies to a credential.

    ``settings.MAX_HOLD``, default 3600s — see ``PLAN_1.8.0.0.md`` G8. Read
    fresh on every call rather than cached, the same reason ``_spread_disabled``
    below reads its switch fresh: the value can change under a running process
    (``/kame set``, a config edit) and a bench sized from a stale number would
    defeat the point of a dial the owner can turn mid-incident.

    Falls back to ``settings.py``'s own declared default rather than a second
    literal here, so the two cannot drift when one is edited and not the other.
    """
    from . import settings

    return settings.number(settings.MAX_HOLD, settings.ALL_NUMBERS[settings.MAX_HOLD])
