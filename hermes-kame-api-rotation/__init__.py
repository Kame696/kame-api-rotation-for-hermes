...
#: The failures a floor is for: the ones about a *credential*, where the host's
#: only other answer is ``credential_pool``'s flat hour. Congestion, timeouts and
#: anything this module could not name are left to the host, which reads several
#: of them better than this one can. See ``_floor_verdict``.
_FLOOR_KINDS = frozenset(
    {"rate_limit", "per_minute", "daily", "insufficient_quota",
     "revoked", "auth", "denied"}
)
...
def register(ctx) -> None:
    ...
    # Before anything that classifies, and outside every guard above, because
    # it is not a decision — it is the evidence the decisions are made from.
    # Hermes' Gemini adapter reads the error body, keeps ErrorInfo and drops
    # the QuotaFailure entry that carries ``quotaId``; without this the window
    # is unreadable and 300 of 300 journal rows say ``unknown``. Failing to
    # install is silent and correct: an older Hermes, a rewritten adapter or a
    # machine with no Gemini all behave exactly as 1.7.0.1 did.
    try:
        from .quota_id_binding import install as _install_quota_id

        _install_quota_id()
    except Exception:
        logger.debug("%s: the quota window stays unreadable", PLUGIN_NAME, exc_info=True)
...
_CONTROL_TICK_S = 1.0
# 1.7.0.1: was 20. The owner saw the chat spinner say "4/14 keys healthy" while
# the status chip under it still read 0/14, and both were true — the spinner is
# computed live at the moment of the call, and the chip reads the last
# snapshot. During a storm a pool empties and refills inside a minute, so a
# twenty-second-old reading is a different reading. Five keeps them close
# enough to agree on screen; the write itself is still skipped when nothing
# changed, so a quiet Hermes does not write four times as often for nothing.
