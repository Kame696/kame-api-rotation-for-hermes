...
_PERMANENT_AUTH_PATTERNS = (
    ...
    # one-api/new-api family gateways (aihubmix and others sharing that
    # envelope), on a 403, documented as its own sentence distinct from
    # "Forbidden - insufficient permissions." (a denial, matched below) and
    # from the billing/model/channel rows on the same status. Narrow on
    # purpose -- "account" immediately followed by "suspended" -- so it does
    # not reach into `carousel.PERMANENT_DENIAL_INDICATORS`' own, differently
    # worded "account has been suspended", which is that module's own
    # evidence for a different bench. Source: research/1.8.0.0/expected/
    # verdicts.jsonl shape doc-aihubmix-403-family --
    # https://docs.aihubmix.com/en/FAQs/HTTP-Codes (the full 403 table).
...
_DENIAL_PATTERNS = (
    ...
    # one-api/new-api family gateways, on a 403: "Forbidden - key(...) allowed
    # only from approved IP ranges." A refusal about the *network* the request
    # came from, not about the key or the model -- distinct from every other
    # row above, and currently otherwise indistinguishable from a dead key
    # (both would defer to the host as a bare 403). Source: research/1.8.0.0/
    # expected/verdicts.jsonl shape doc-aihubmix-403-family --
    # https://docs.aihubmix.com/en/FAQs/HTTP-Codes (the full 403 table).
...
_BILLING_PATTERNS = (
    ...
    # Anthropic, on a 400: "You have reached your specified API usage limits
    # for this API." A user-set spend cap, not a malformed request -- and not
    # ``_QUOTA_PATTERNS``' own "usage limit reached" either, because this
    # sentence says the words in the opposite order ("reached ... usage
    # limits") and pluralises "limits", so it never collided with that
    # pattern and needs its own row rather than a widened one. Source:
    # research/1.8.0.0/expected/verdicts.jsonl shape
    # doc-anthropic-402-529-400 -- https://docs.anthropic.com/en/api/errors
    # ("The API also returns a 400 when usage reaches an organization or
...
# google.rpc.Code.RESOURCE_EXHAUSTED's own canonical description, sent
# verbatim by Gemini/Vertex whenever the body carries no QuotaFailure, no
# RetryInfo and no ErrorInfo at all -- the plugin's single most common unsized
# 429, 688 occurrences in nine days on this machine (research/1.8.0.0/
# expected/verdicts.jsonl shape real-01). "has been" sits between the two
# words ``_QUOTA_PATTERNS``' bare "resource exhausted" stem requires adjacent,
# so it needs its own pattern rather than a widened one, which would also
# start matching sentences that stem was deliberately kept narrow to avoid.
# Named separately (not inlined in ``_QUOTA_PATTERNS`` below) because
# ``classify()`` also reads it directly, to answer a 429 that carries this
# exact sentence and nothing else machine-readable at all -- see the comment
# where it is consulted there. Sources: https://github.com/googleapis/
# googleapis/blob/master/google/rpc/code.proto ("RESOURCE_EXHAUSTED: Some
# resource has been exhausted, perhaps a per-user quota") and
# https://cloud.google.com/vertex-ai/generative-ai/docs/model-reference/api-errors?hl=en.
_RESOURCE_EXHAUSTED_BOILERPLATE = re.compile(
    r"resource[\s_-]*(?:has|have)[\s_-]*been[\s_-]*exhausted", re.I
)
...
_QUOTA_PATTERNS = (
    ...
    _RESOURCE_EXHAUSTED_BOILERPLATE,
...
def looks_like_upstream_wrapper(body: Any) -> bool:
    ...
    here. Signals include the envelope phrase, explicit upstream source/type,
    and metadata carrying a nested raw error or upstream provider name.
    """
...
def _response_text(error: Any, limit: int = 20000) -> str:
    ...
    **1.7.0.0: this function used to give up on the only case it was written
    for.** 1.6.0.3 read ``.text`` and ``.content``, and stopped there on the
    reasoning that ``ResponseNotRead`` "does no I/O, so this can neither block
    a call nor consume a body somebody else still needs". Every chat request
    Hermes makes is a *streaming* request, and a streamed response raises
    exactly that from both properties — so across 450 refusals in the owner's
    own logs, ``quotaId`` was recovered **zero** times. A daily cap arrived
    every time wearing a 29-second retry delay, and fourteen keys that were
    out of requests until midnight were re-probed every twenty seconds all
    night.

    ``response.read()`` returns it. Measured on the real endpoint, 04/09/2026:
    ``.text`` raises ``ResponseNotRead``; ``.read()`` returns 1363 bytes
    carrying ``GenerateRequestsPerDayPerProjectPerModel-FreeTier``. It is safe
    here for reasons that are properties of this path rather than hopes about
    it: this is an *error* response, so there is no answer streaming behind it
    to consume; Hermes has already finished with it (``gemini_http_error``
    parses the body and keeps four fields before constructing the exception
    this reads); and a refusal payload is on the order of a kilobyte. It is
    still wrapped, and a failure still returns ``""``.
    """
    if error is None:
        return ""
    try:
    ...
    if response is None:
        return ""
    for attribute in ("text", "content", "read"):
        try:
            raw = getattr(response, attribute, None)
            if attribute == "read":
                # The streamed case, and the only one that ever happens on a
                # chat call. Last, so a body already in memory is always
                # preferred to one that has to be pulled off the wire.
                raw = raw() if callable(raw) else None
        except Exception:
...
def resolve_evidence_body(body: Any = None, error: Any = None) -> Any:
    """One precedence order for classification and dispatch ownership checks."""
    return body or _response_body(error) or _details_body(error)


def _surface_reading(body: Any, error: Any):
    from .provider_rules import api_surface, reading
    return reading(api_surface(error), body)
...
def classify(
    *,
    ...
    evidence_body = resolve_evidence_body(error_body, error)
    ...
    #    get out of the way.
    # SDK-only payloads retain the same ownership as an explicit body. Reading
    # their prose without their envelope could retire the aggregator's key for
    # an upstream credential failure.
    if looks_like_upstream_wrapper(evidence_body):
        return None
    ...
    catalog_reading = (
        _surface_reading(evidence_body, error)
        or catalog.look_up(*structured_error_values(evidence_body, error, error_code))
        or status_reading
    )
    ...
    # Weaker still than the class name above, and consulted only when neither
    # a field nor a class spoke: Google/Vertex's own gRPC boilerplate for
    # RESOURCE_EXHAUSTED, sent whenever the response carries no QuotaFailure,
    # no RetryInfo and no ErrorInfo at all (research/1.8.0.0/expected/
    # verdicts.jsonl shape real-01, 688 real occurrences). This is not the
    # ambiguous "exceeded your current quota ... billing" sentence a few lines
    # down — it carries no billing wording of any kind — so reading it as an
    # unsized throttle cannot reintroduce the regression the "429 alone is not
    # enough" rule below exists to prevent (host_corpus.py's "Monthly quota
    # reached." / "usage limit reached" counter-examples are different
    # sentences and are unaffected).
    if catalog_reading is None and _matches((_RESOURCE_EXHAUSTED_BOILERPLATE,), message, body_text):
        catalog_reading = catalog.Reading(
            catalog.THROTTLE,
            why="Google/Vertex's own generic RESOURCE_EXHAUSTED wording, with nothing else to size it by",
        )

    # RED_TEAM.md F2 (2026-09-19): set only when the wastebasket-TERMINAL
    # branch just below lets an uncertain reading fall through instead of
    # returning. Gates the two prose checks that must never fire on that
    # fall-through — see the comment at each one for why.
    wastebasket_terminal_fallthrough = False
    catalog_throttle = False
    if catalog_reading is not None:
        family = catalog_reading.family
        if family == catalog.AUTH_REFRESH:
            key_specific = tuple(p for p in _PERMANENT_AUTH_PATTERNS if "authentication" not in p.pattern)
            if _matches(key_specific, message, body_text):
                family = catalog.AUTH_DEAD
                catalog_reading = catalog.Reading(catalog.AUTH_DEAD,
                    why="explicit invalid-key evidence outranks a refresh-capable status")
            else:
                return None
        if family in (catalog.SERVER, catalog.TIMEOUT, catalog.TERMINAL,
                      catalog.NOT_A_FAILURE):
            # The wastebasket half of TERMINAL (``invalid_request_error``/
            # ``INVALID_ARGUMENT``, ``certain=False``) is a family the
            # provider filed a bare 400 under, not a fact about this request
            # -- catalog.py's own docstring: "whenever a payload carries one
            # of these *and* something specific, the specific one is the
            # fact". Anthropic's 400 spend-limit refusal is exactly that
            # payload: `type: invalid_request_error` with nothing else
            # structured, and the specific fact is Anthropic's own prose,
            # never read while this branch returned unconditionally. A
            # *certain* reading of any of these four families still ends the
            # turn here, unchanged -- only the uncertain wastebasket now
            # yields to the billing prose check below, falling through to the
            # same ``None`` that check already reaches when nothing there
            # matches either. Source: research/1.8.0.0/expected/
            # verdicts.jsonl shape doc-anthropic-402-529-400.
            #
            # RED_TEAM.md F2 (2026-09-19): the fall-through used to reach
            # every prose check below, including permanent-auth and denial —
            # not just billing. R43 is a closed rule in this project
            # precisely because a provider can quote the user's own prompt
            # back inside an error body, and a wastebasket 400 is a shape
            # where the "specific fact" this branch exists to surface is
            # untrusted prompt text, not provider evidence. Measured: a
            # malformed request whose text happened to contain the ordinary
            # English phrase "account suspended" reached ``auth_permanent``
            # -> ``revoked`` -> immediate retirement, with no strike guard,
            # for a fault that was never about a credential. Retirement
            # requires structured evidence (a code/field the provider set),
            # never prose alone -- so this fall-through may only ever reach
            # the billing check just below; ``wastebasket_terminal_
            # fallthrough`` gates the permanent-auth and denial prose checks
            # out of it explicitly, rather than relying on every future
            # prose table added below to remember the exclusion on its own.
            if family == catalog.TERMINAL and not catalog_reading.certain:
                wastebasket_terminal_fallthrough = True
            else:
                return None
    ...
    #
    # RED_TEAM.md F2: never reachable from the wastebasket-TERMINAL
    # fall-through above. That fall-through exists so Anthropic's own
    # spend-limit sentence can be read from a bare 400; it must not also
    # hand this pattern table the user's own prompt text, which a provider
    # can echo back verbatim (R43) and which retires a credential the moment
    # it happens to contain an ordinary phrase like "account suspended".
    if not wastebasket_terminal_fallthrough and _matches(_PERMANENT_AUTH_PATTERNS, message, body_text):
        return Verdict(
            reason="auth_permanent",
            retryable=False,
            should_rotate_credential=True,
    ...
    # The ambiguous sentence, and the guard that makes it survivable.
    #
    # Google's free-tier 429 says OpenAI's out-of-credits words for a
    # twenty-one second throttle. Removing this branch entirely was tried and
    # measured: the plugin then answered ``rate_limit`` for every shape that
    # wears those words, including a plain 402 and an aggregator's "insufficient
    # credits" — and because a plugin hook answers at step 0 of Hermes'
    # pipeline, answering *is* overriding. Four of the host's own classifier
    # tests failed, all of them cases where the host was right and this module
    # was not.
    #
    # So the branch stays, with the guard it always had: only when nothing else
    # named a wait, and only when the catalogue did not call it a throttle. What
    # changed is the cost of being wrong here. A billing verdict used to end the
    # turn; ``_to_hook_result`` now makes anything the pool can rotate past
    # retryable, so the same misreading costs a rotation instead of an answer.
    ambiguous = _matches(_AMBIGUOUS_BILLING_PATTERNS, message, body_text)
    if (_matches(_BILLING_PATTERNS, message, body_text)
        and not (catalog_reading is not None and catalog_reading.billing_context_only)) or (
        ambiguous
    ...
    #
    #    RED_TEAM.md F2: same exclusion as step 1, same reason — the
    #    wastebasket-TERMINAL fall-through carries untrusted prompt text,
    #    not provider evidence, and this table must not read it either.
    if not wastebasket_terminal_fallthrough and _matches(_DENIAL_PATTERNS, message, body_text):
        return Verdict(
            reason="auth",
            retryable=False,
            should_rotate_credential=True,
    ...
    tokens = structured_error_tokens(evidence_body, error, error_code, error_type)
    ...
    return Verdict(
        ...
        should_rotate_credential=True,
        reset_at=decision.reset_at,
        window_scoped_reset=decision.window_scoped_reset,
        quota_window=decision.window,
        quota_scope=decision.scope,
        source=decision.source,
        rationale=decision.rationale,
