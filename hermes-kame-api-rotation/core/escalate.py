...
#
# 1.8.0.0: this stays a 24h sanity bound on the *widening this function
# invents*, deliberately not lowered to the owner's operational ceiling
# (``settings.MAX_HOLD``, default 1h, see ``PLAN_1.8.0.0.md`` G8). This
# module is framework-free — numbers in, a number out, no settings, no
# clock but the one it is handed — and stays that way; the ceiling is
# applied by the only caller, ``pool_binding._remember``, to the deadline
# this produces before it is ever recorded. Changing this constant would
# duplicate that bound in a module that has no way to hear the owner turn
# the dial.
...
_WINDOW_ESCALATION_CAPS: Dict[str, float] = {
    ...
    # A tokens-per-minute counter recovers on the same sixty-second wall an
    # ordinary per-minute one does (see quota.QuotaWindow.TOKENS_PER_MINUTE);
    # only the unit being counted differs, and this cap is about the *clock*,
    # not the unit. Omitting it would leave a stuck TPM key free to escalate
    # to a full day the way an unnamed window can — exactly the gap this
    # dict exists to close for PER_MINUTE, reopened one member over.
    QuotaWindow.TOKENS_PER_MINUTE: MAX_PER_MINUTE_HOLD_SECONDS,
...
def factor_for(strikes: int) -> float:
    ...
    if strikes < STRIKES_BEFORE_STRETCHING:
        return 1.0
    # Saturation must precede the power: otherwise a long-lived streak raises
    # OverflowError even though the final multiplier has always been bounded.
    import math
    if strikes - 1 >= math.log2(MAX_FACTOR):
        return MAX_FACTOR
    return 2.0 ** (strikes - 1)
...
def stretch(
    *,
    ...
    """
    # 1.7.0.7 put a blanket `if provider_timed(source): return None` here —
    # any current refusal carrying a fresh provider number was refused
    # escalation outright, whatever `strikes` said. Decision 0004 D3
    # (measured on the owner's corpus): that guard blocked 31 of ~530
    # widenings, and every one of those 31 was exactly the case R13
    # authorizes — a stated deadline served in full, refused again. Removed.
    # `strikes` below is what still decides: repetition alone never
    # multiplies a stated number (R10/R12), but two deadlines proven short
    # in a row — `journal.short_streak` — does.
    if strikes < STRIKES_BEFORE_STRETCHING:
        return None
