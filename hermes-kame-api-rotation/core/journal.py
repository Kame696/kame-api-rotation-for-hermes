...
def short_streak(
    ...
    *,
    ...
    """
    from .vocabulary import internal

    # 1.7.0.7 put a blanket `if provider_timed(source): return 0` here — any
    # refusal in flight carrying a fresh provider number could never itself
    # complete a strike, whatever the history behind it showed. Decision
    # 0004 D3 (measured on the owner's corpus): that guard, together with
    # the matching one in the loop below, blocked 31 of ~530 widenings —
    # and every one of those 31 was exactly the case R13 authorizes: a
    # stated deadline served in full, refused again. Removed. What still
    # decides is the sequence below: `_landed_right_after`, same window,
    # same failure family, no success in between. Repetition alone never
    # multiplies a stated number (R10/R12) — proof that a wait was served in
    # full and was wrong anyway is not repetition, it is the measurement
    ...
    for previous in reversed(history):
        # Do not bridge a different failure family, or a calendar anchor and
        # an estimated duration, merely because both say the same window —
        # those two breaks are 1.7.0.7's and stay: they stop a streak being
        # assembled from unrelated refusals. A third break lived here too,
        # `if provider_timed(previous.source): break` — removed by decision
        # 0004 D3, because a *previous* block being provider-timed is not
        # evidence against it: it is the shape of the exact case R13
        # authorizes, a stated deadline this plugin waited out in full,
        # refused again. Blocking the chain there blocked the strike.
        if reason is not None and internal(previous.reason) != internal(reason):
            break
        if source is not None and (previous.source == "anchor") != (source == "anchor"):
            break
