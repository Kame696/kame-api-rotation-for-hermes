...
from .quota import QuotaScope, QuotaWindow, TOKEN_UNIT_MARKERS
...
AUTH_REFRESH = "auth_refresh"  # route-scoped only (provider_rules.py) — see F1
...
class Reading:
    ...
    def __init__(
        ...
        *,
        ...
        # Endpoint-reviewed code says billing concerns optional extra usage,
        # not the included allowance's ability to recover at its reset.
...
_put(
    ...
    # envelope), on a 403. A distinct code from OpenAI's `insufficient_quota`
    # above -- confirmed not a substring match of it by test_v1_7_0_0.py --
    # so it needs its own row rather than relying on the prose ("out of
    # credit") that usually rides beside it. Documented, not yet measured on
    # this machine as the code alone: research/1.8.0.0/expected/verdicts.jsonl
    # shape doc-aihubmix-403-family, sourced from
    # https://docs.aihubmix.com/en/FAQs/HTTP-Codes and
    # https://github.com/songquanpeng/one-api/blob/main/relay/controller/helper.go
    # ("insufficient_user_quota" is HTTP 403).
...
_put(
    ...
    # OpenAI's "acceleration limit": the request RATE grew too fast, a
    # different fact from a full counter but the same move -- rotate, and the
    # sizing cascade still applies. Anthropic and Vertex document the same
    # behaviour with no code of their own (research/1.8.0.0/expected/
    # verdicts.jsonl shape doc-openai-slow-down), so only the one provider
    # that actually names it in a field gets a row here (R01: evidence, not
    # a guess extended to providers that never said anything).
    # Source: https://platform.openai.com/docs/guides/error-codes
    # ("429 - Slow down | Code: slow_down | Your request rate increased too
    # quickly").
    "slow_down",
)
...
#
# RED_TEAM.md F1 (2026-09-19): 1.8.0.0 briefly split this row, moving
# ``authentication_error``/``authentication``/``invalid_authentication``/
# ``UNAUTHENTICATED``/``gemini_unauthorized`` out to a new ``AUTH_REFRESH``
# family below, deleting the comment that used to sit here ("A 401 from
# Google is the key, not the request") without anything in ``decisions/``
# recording the replacement. The measured cost: a real Gemini 401
# ("Request had invalid authentication credentials.", Google's own standard
# ``UNAUTHENTICATED`` wording) stopped rotating and instead reached
# ``dispatch_binding.py``'s ``"raise", "auth_refresh"`` exit — ending the
# turn with sixteen healthy keys still idle, a regression against invariant
# I3 (nothing KAME does may end worse than not trying).
#
# ``AUTH_REFRESH`` is real for exactly one narrow case: a surface where the
# host can genuinely refresh an OAuth session rather than replace a key —
# Vertex, per ``core/provider_rules.py``'s route-scoped ``google_vertex``
# rule, which is unaffected by this table and still returns ``AUTH_REFRESH``
# for that surface alone. On every other, unscoped surface a bare 401/400
# carrying one of these labels goes back to being an ordinary dead
# credential: rotate to the next key, exactly as 1.7.0.5 did before this
# family existed.
_put(
    ...
    "gemini_unauthorized",
    # Google's rejection of a legacy Standard API key, on a 401 --
    # ErrorInfo.reason ACCESS_TOKEN_TYPE_UNSUPPORTED, message "... expected
    # OAuth 2 access token". Distinct from the generic UNAUTHENTICATED status
    # the same 401 also carries, now folded back into this same AUTH_DEAD
    # reading (see the note above): the message *reads* like a refreshable
    # OAuth problem, but the key TYPE is what is rejected, so it will never
    # clear on its own the way an expired bearer token does.
    # structured_error_values() ranks the ErrorInfo `reason` bucket ahead of
    # the `status` family bucket, so this row wins the tiebreak the same way
    # API_KEY_INVALID already does against INVALID_ARGUMENT. Sources:
    # research/1.8.0.0/expected/verdicts.jsonl shape
    # doc-gemini-401-standard-key --
    # https://ai.google.dev/gemini-api/docs/api-key?hl=en ("Unrestricted keys
    # rejected: The Gemini API rejects requests from unrestricted standard
    # keys") and the host's own
    # agent/gemini_native_adapter.py:is_standard_key_auth_error.
...
def read_quota_id(body_text: str) -> Tuple[str, str]:
    ...
    # Same promotion ``quota.detect_quota_window`` makes, on the same
    # evidence, for the sizing cascade that reads a ``quotaId`` with nothing
    # to size it by (``classify.py``'s ``catalog_throttle`` branch). Google's
    # own ``GenerateContentInputTokensPerModelPerMinute`` names both facts in
    # one string; reading this one and not that one would answer differently
    # depending on which of the two paths a TPM refusal happened to take.
    if window == QuotaWindow.PER_MINUTE and any(marker in raw for marker in TOKEN_UNIT_MARKERS):
        window = QuotaWindow.TOKENS_PER_MINUTE
