...
EXPECTED_RESTS = (
    ("timeout", 0.0, "the provider went quiet; the key is fine, immediately selectable again"),
    ("server", 1.0, "a 5xx is the provider's problem, not the key's"),
    # 1.8.0.1: 20s -> 45s, on a curve that turned out to double-count
    # retries. 1.8.0.2: recounted once per retry over the whole real corpus
    # (471 retries following a bare 429) and corrected to 30s — the argmax,
    # avoiding 388 of 413 avoidable refused calls at the cost of delaying 6
    # successes. See core.carousel.UNSIZED_THROTTLE_REST_S for the full
    # measurement and the correction, and settings.UNSIZED_THROTTLE_REST
    # (KAME_UNSIZED_REST) for the dial that lets 30s be tried against 0.
    # 1.8.1.0: the "none" case is now an experiment on by default — 1s, then
    # 2, 4, 8 ... per consecutive refusal on the same key and model, bounded
    # by max_hold_seconds — instead of the flat 30s. ``unsized_throttle_
    # backoff`` off (KAME_UNSIZED_BACKOFF=0) puts the flat 30s back. The
    # numeric column stays ``None`` because the number depends on the streak
    # and on whether the provider named one, not on the kind alone.
    ("per_minute", None, "the provider's own retry hint, or 30s when it gives none; a bare RESOURCE_EXHAUSTED instead doubles 1s..64s per repeat (off = flat 30s)"),
    ...
    ("daily", 300.0, "the label alone: a re-probe. The hour needs a silent pool"),
...
def rest_lines(kinds: Sequence[Any]) -> List[str]:
    ...
    out.append("")
    out.append("    Almost nothing escalates any more, and that is deliberate.")
    out.append("    A rest is a number the provider stated, or a flat re-probe")
    out.append("    when it stated none. Repeating a refusal is not evidence")
    out.append("    that the provider's number was wrong.")
    out.append("")
    out.append("    The one ladder left is for a key the provider REFUSES —")
    out.append("    401/403 — where 20s, 40s, 80s ... is how KAME counts to")
    out.append("    'this credential is not coming back' before retiring it.")
    out.append("")
    out.append("    A rate limit stopped climbing in 1.6.0.3: the provider")
    out.append("    recomputes its own wait on every refusal, so restating it")
    out.append("    is the rolling window working, not a lie to correct.")
    out.append("")
    out.append("    A server error stopped climbing in 1.7.0.4, for a simpler")
    out.append("    reason: a 5xx is not metered. It costs no quota, so a")
    out.append("    longer rest buys nothing and only holds back a key that")
    out.append("    was never at fault. Measured on real keys, the old ladder")
    out.append("    climbed 7 times out of 16 while the pool was still")
    out.append("    answering — once benching a healthy key for 40 seconds.")
    out.append("")
    out.append("    A DAILY label is the exception, since 1.7.0.2. It is read as")
    out.append("    evidence and not as proof, because measured on real keys a")
    out.append("    key refused for the day answered again 6 to 36 minutes later,")
    out.append("    21 times out of 21. So it rests for a re-probe while ANY key")
    out.append("    on that model is still answering, and only buys the full")
    out.append("    daily cooldown once the whole pool has gone quiet for 20")
    out.append("    minutes. A deadline the provider genuinely names still wins.")
    out.append("")
