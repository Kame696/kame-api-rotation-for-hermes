...
from .core import evidence, multikey, shared_health, stitch, vocabulary
...
from .core.classify import classify, Verdict, looks_like_upstream_wrapper, resolve_evidence_body
...
def candidates(agent: Any) -> Tuple[List[str], Dict[str, Any]]:
    ...
    * the single resolved key, which is not a pool at all — the carousel still
    ...
    # Rows that carry exactly one key claim it first; a row that carries a
    # *list* is only the fallback for parts nobody else owns.
    #
    # Order is the whole fix here. ``pool_binding._expand`` rebuilds the pool
    # as the parent row **plus** one child row per key, and the child is the
    # row whose id the journal counts under. Iterating in pool order put the
    # parent first, and because ``_add`` keeps the first claimant, the parent
    # took all fourteen parts and every child was skipped as already-seen.
    # The keys were right and the identities were wrong: fourteen credentials
    # wearing one id, which is what ``core.journal`` reads, which is what
    # decides whether ``escalate.stretch`` ever sees a second strike. It never
    # did. See ``counted_as`` for the two ways that failed silently.
    ...
    for key, entry in listed:
        # Splitting can be off, or a part can be new since the last expansion.
        # Then this row is still the only thing that knows the key, and it
        # supplies it exactly as before — ``counted_as`` names the part.
        _add(key, entry)
...
class _Spinner:
    ...
    * **Interval** — at least ``interval`` seconds between updates, chosen by
    ...
    #: The shortest gap allowed between two **different** lines. 1.7.0.2, and
    #: it exists to fix one wrong line on screen.
    #:
    #: The interval guard below compares the time since the last frame against
    #: the cadence the *new* frame asks for, and that is wrong across a
    #: transition. A wait that ends draws ``next key in 0s`` at the one-second
    #: cadence a near-zero countdown deserves; if the keys then refuse at once
    #: and a fresh hour-long wait opens, the new frame asks for the 30-second
    #: cadence an hour-out countdown deserves — and is suppressed for thirty
    #: seconds, leaving ``0s`` on screen while the truth is 58 minutes.
    #:
    #: The owner photographed exactly that: the chat line reading
    #: ``next key in 0s`` beside a status bar reading ``0/14 58m 43s``, eleven
    #: seconds into the new wait. Two readings of one clock, and the one he was
    #: looking at was the stale one.
    #:
    #: A line whose text *changed* carries new information by definition, so
    #: the flood the interval guard exists to prevent is a flood of identical
    #: text — which ``_REFRESH_S`` already governs. What a changed line needs
    #: is a floor, not a cadence: one second, which is also the fastest cadence
    #: ``cadence_for`` ever asks for, so nothing draws more often than the
    #: countdown it is drawing could change.
    ...
    def update(cls, agent: Any, text: str, *, interval: Optional[float] = None) -> None:
        ...
        with cls._lock:
            ...
            if text == last_text and now - last_say < cls._REFRESH_S:
                return
            # See ``_CHANGED_FLOOR_S``: a changed line is a state change, and
            # gating it by a cadence chosen for the new number is how a
            # finished countdown stays on screen after a new one began.
...
class DispatchBinding:
    ...
    def run(
        ...
        while True:
            ...
            if status == "EXHAUSTED":
                ...
                recovered = self._wait_for_recovery(agent, identity, keys, vigil)
                ...
                if not recovered:
                    ...
                    return original(agent, api_kwargs, *args, **kwargs)
                slept = True
                # Waiting may have ended because the live pool gained a key.
                # Refresh its entry mapping as well as the key list so the
                # next failure is attributed to the credential actually used.
                try:
                    keys, entry_by_key = candidates(agent)
                except Exception:
                    logger.debug("kame: could not refresh candidates after wait", exc_info=True)
                continue
            ...
            # What the line says, and what it deliberately stopped saying.
            #
            # It used to end with ``on key {attempt}`` — and ``attempt`` counts
            # rotations in this turn, not keys. With fourteen keys the owner
            # watched it read "on key 32", a number that cannot mean anything to
            # anyone. The count was never compared to the pool size because it
            # was never a key index.
            #
            # He asked for the health count and, at most, a word saying a
            # rotation is happening. That is what is here: the same sentence
            # either way, with the symbol carrying the difference, and one short
            # tail that says what is going on rather than counting something the
            # reader cannot use.
            ...
            progress = _Progress()
            # Read only by ``timings``. ``started`` above is the whole turn;
            # this is this attempt. Their difference INCLUDES previous
            # provider calls; actual recovery-wait time is tracked separately.
            ...
            try:
                ...
                verdict, kind, status = self._on_failure(
                    ...
                    credential_id=counted_as(entry_by_key.get(key), key),
                )
                # The attempt is over and its verdict is known. Written here
                # rather than at each of the three branches below so a branch
                # added later cannot be the one that forgets. Decides nothing.
                timings.record(
                    identity=identity,
                    ...
                    call_id=call_id,
                    chars_seen=len(seen),
                    # 1.7.0.2. The field existed from the day `timings` shipped
                    # and no caller ever filled it, so every one of the owner's
                    # 2,458 recorded attempts carried `rest_s: null` and the
                    # one number that explains a stall — how long this refusal
                    # cost the key — could only be recovered by opening
                    # `state.json` and reading the event ring by hand.
                    #
                    # Read back off the engine rather than passed down from
                    # `_on_failure`, on purpose: what belongs in the log is the
                    # rest that was *stored*, which is not always the rest that
                    # was asked for — a cap can bite, and a longer existing
                    # cooldown always wins.
                    rest_s=self.engine.next_recovery_seconds(identity, [key]),
                    # 1.8.1.0. Same reasoning as ``rest_s`` immediately above:
                    # read back off the engine rather than passed down from
                    # ``_on_failure``, so ``calls.jsonl`` can tell a rest the
                    # ladder produced from a flat or stated one of the same
                    # size. Empty string, not ``None``, when nothing sized it
                    # this way — ``timings.record`` already treats a falsy
                    # value as "nothing to write" for every other field here.
    ...
    def _wait_for_recovery(
        ...
        while slept < wait:
            ...
            slept += slice_s
            try:
                live_keys, _ = candidates(agent)
                if any(key not in keys for key in live_keys):
                    break
            except Exception:
                logger.debug("kame: could not inspect candidates during wait", exc_info=True)
            # Another completed call or an explicit pool reset can release a
            # key before the deadline observed at entry. Re-select instead of
            # sleeping out a stale minute-long pass. Ordinary expiry retains
            # the existing padding/jitter policy; this is early recovery only.
...
def install(module: Optional[Any] = None) -> Optional[DispatchBinding]:
    ...
    try:
        ...
        # RED_TEAM.md F3 (2026-09-19): the owner's ceiling dial
        # (``KAME_MAX_HOLD`` / ``settings.MAX_HOLD``) was read and clamped
        # correctly by ``settings.py`` and by ``pool_binding._ceiling_s()``,
        # but nothing ever pushed it into the one ``Carousel`` this binding
        # actually marks and selects against — ``core/carousel.py``'s
        # module-level ``ENGINE`` singleton is built with the hard-coded
        # ``MAX_HOLD_S = 3600.0`` default, and this was the only place a host
        # setting has ever reached that engine (see ``daily_cooldown_s``
        # immediately above, wired the same way since before this release).
        # Measured: ``KAME_MAX_HOLD=120`` left ``ENGINE.max_hold_s`` at
        # 3600.0, so lowering the dial did nothing to the carousel while
        # ``pool_binding``'s host-block path obeyed it — the two halves of
        # ``decisions/0005``'s "single point" disagreeing with each other.
        binding.engine.max_hold_s = settings.number(
            settings.MAX_HOLD, binding.engine.max_hold_s
        )
        # 1.8.0.1. Same wiring, same reason F3 above states it for
        # ``max_hold_s``: a setting declared in ``settings.py`` and offered in
        # ``plugin.yaml`` is decorative until something pushes its resolved
        # value onto the one ``Carousel`` this binding actually marks and
        # selects against. ``settings.UNSIZED_THROTTLE_REST``
        # (``KAME_UNSIZED_REST``) is the owner's dial for testing his own
        # hypothesis against the flat 45s default — see
        # ``core.carousel.UNSIZED_THROTTLE_REST_S`` for the measurement — and
        # zero is a real position on it: set to 0 an unsized throttle is
        # offered again immediately, which is the "spin" comparison the
        # measurement argues against but the dial has to be able to run.
        binding.engine.unsized_throttle_rest_s = settings.number(
            settings.UNSIZED_THROTTLE_REST, binding.engine.unsized_throttle_rest_s
        )
        # 1.8.1.0. Same wiring, same reason F3/the paragraph above state it:
        # a setting declared in ``settings.py`` and offered in
        # ``plugin.yaml`` is decorative until something pushes its resolved
        # value onto the one ``Carousel`` this binding actually marks and
        # selects against. ``settings.UNSIZED_THROTTLE_BACKOFF``
        # (``KAME_UNSIZED_BACKOFF``) is the owner's switch for the doubling
        # experiment — see ``core.carousel.Carousel._escalate`` for the
        # ladder and for the measurement this experiment runs ahead of.
        binding.engine.unsized_throttle_backoff = settings.is_on(
            settings.UNSIZED_THROTTLE_BACKOFF
        )
        # 1.8.1.0. Where the ladder stops growing; same wiring for the same
        # reason. Without this push the dial would sit decorative (F3).
        binding.engine.unsized_backoff_max_s = settings.number(
            settings.UNSIZED_BACKOFF_MAX, binding.engine.unsized_backoff_max_s
        )
        # RED_TEAM.md F4 (2026-09-19): ``core.shared_health.enabled()`` reads
        # only ``os.environ`` (it cannot import this module — ``core`` has no
        # import of the host layer, see that module's own docstring), and
        # ``settings.load()`` never exported the variable, so the config
        # default (``DEFAULTS_ON`` — on since G4 passed) was inert in both
        # directions: a fresh install with nothing in the environment shipped
        # the feature OFF while the panel and ``plugin.yaml`` both said ON.
        # Pushing the resolved value in here, once, the same way
        # ``max_hold_s``/``daily_cooldown_s`` are pushed above, lets
        # ``settings.is_on`` decide (environment first, then config, then
        # ``DEFAULTS_ON``) without ``core`` ever importing it. The live
        # environment variable still overrides on every call after this,
        # because ``shared_health.enabled()`` checks it first and only falls
        # back to this pushed value when it says nothing at all.
